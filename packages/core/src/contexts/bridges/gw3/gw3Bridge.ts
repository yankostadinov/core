...
import { GW3ContextData as ContextData } from "./gw3ContextData";
import { applyContextDelta, deepEqual, deepClone } from "../../helpers";
import * as msg from "./gw3Messages";
import { ContextMessage } from "./gw3ContextMessage";
...
export class GW3Bridge implements ContextBridge {
    ...
    private _connection: Glue42Core.Connection.GW3Connection;
    ...
    public constructor(config: Glue42Core.Contexts.ContextsConfig) {
        this._connection = config.connection as Glue42Core.Connection.GW3Connection;
        ...
        return this._gw3Session
            ...
            .then((createContextMsg: ContextMessage) => {
                ...
                if (!this._contextIdToName[createContextMsg.context_id]) {
                    ...
                    const contextData = this._contextNameToData[name] || new ContextData(createContextMsg.context_id, name, true, null);
        ...
        if (updatedMessageType === msg.GW_MESSAGE_JOINED_ACTIVITY) {
            ...
            if (!contextData || !contextData.isAnnounced) {
                if (updatedMessageType === msg.GW_MESSAGE_SUBSCRIBED_CONTEXT) {
                    ...
                    contextData = contextData || new ContextData(contextId, contextUpdatedMsg.name, true, null);
                    contextData.sentExplicitSubscription = true;
    ...
    private calculateContextDelta(from: any, to: any): Glue42Core.Contexts.ContextDelta {
        const delta: Glue42Core.Contexts.ContextDelta = { added: {}, updated: {}, removed: [], reset: null };
        if (from) {
